...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
            ...
            sql = sqlalchemy.sql.text('''
                insert into users 
                    (id_users, created_at, id_urls, friends_count, listed_count, favourites_count, 
                    statuses_count, protected, verified, screen_name, name, location, description)
                    values
                    (:id_users, :created_at, :id_urls, :friends_count, :listed_count, :favourites_count,
                    :statuses_count, :protected, :verified, :screen_name, :name, :location, :description)
                on conflict do nothing;
                ''')
            user = tweet['user']
            res = connection.execute(sql,{'id_users':user['id'], 'name':remove_nulls(user['name']), 
                'screen_name':remove_nulls(user['screen_name']), 'location':remove_nulls(user['location']), 
                'description':remove_nulls(user['description']), 'protected':user['protected'], 'verified':user['verified'], 
                'friends_count':user['friends_count'], 'favourites_count':user['favourites_count'], 
                'listed_count':user['listed_count'], 'created_at':user['created_at'], 'id_urls':user_id_urls, 
            ...
            sql=sqlalchemy.sql.text(f'''
                insert into tweets
                    ...
                    retweet_count, favorite_count, quote_count, source, text, country_code, state_code, lang, place_name, geo)
                    values
                    (:id_tweets, :id_users, :created_at, :in_reply_to_status_id, :in_reply_to_user_id, 
                    :quoted_status_id, :retweet_count, :favorite_count, :quote_count, :source, :text, 
                    :country_code, :state_code, :lang, :place_name, :geo)''')
            geo = geo_str + '(' + geo_coords + ')' 
            res = connection.execute(sql, {'id_tweets':tweet['id'], 'id_users':tweet.get('in_reply_to_user_id', None), 
                'created_at':tweet['created_at'], 'in_reply_to_status_id':tweet['in_reply_to_status_id'], 
                'in_reply_to_user_id':tweet['in_reply_to_user_id'], 'quoted_status_id':tweet.get('quoted_status_id', None), 
                'retweet_count':tweet['retweet_count'], 'favorite_count':tweet['favorite_count'], 'quote_count':tweet['quote_count'], 
                'source':remove_nulls(tweet['source']), 'text':remove_nulls(text), 'country_code':country_code,
                'state_code':state_code, 'lang':remove_nulls(tweet['lang']),'place_name':remove_nulls(place_name), 'geo':geo})
        ...
        print("ERROR: self._transaction is", connection._transaction, "but should be None")
